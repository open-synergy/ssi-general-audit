...
class GeneralAuditWsC7d5f2bDataComparison(models.Model):
    """Comparison line for a Physical Check Audit Procedure worksheet.
    ...
    _name = "general_audit_ws_c7d5f2b.data_comparison"
    _description = "Physical Check Data Comparison"
    ...
    def _compute_name(self):
        """Derive the display name from the selected comparison source.

        :return: nothing; assigns ``name`` from the title of the
            selected General Ledger or Subledger, or ``False`` when
            neither is selected.
        """
        for record in self:
