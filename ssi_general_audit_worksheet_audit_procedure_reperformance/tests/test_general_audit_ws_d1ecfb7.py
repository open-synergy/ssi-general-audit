...
class TestGeneralAuditWsD1ecfb7(YamlTransactionCase):
    def test_general_audit_ws_d1ecfb7(self):
        self.run_yaml_scenario("test_data_general_audit_ws_d1ecfb7.yaml")
    ...
    def _create_worksheet_fixture(self):
        ...
        client = (
            ...
            .with_user(admin)
            .create(
                {"name": "Test Audit Client - D1ECFB7 Onchange", "is_company": True}
            )
        )
        accountant = (
            env["res.partner"]
            .with_user(admin)
            .create({"name": "Test Audit Accountant - D1ECFB7 Onchange"})
        )
        ...
        env["res.partner.id_number"].with_user(admin).create(
            {
                ...
                "name": "CPA-D1ECFB7-ONCHANGE-0001",
            }
        )
        account_type_set = (
            env["client_account_type_set"]
            .with_user(admin)
            .create({"name": "Test Account Type Set - D1ECFB7 Onchange", "code": "/"})
        )
        standard = (
            env["accountant.financial_accounting_standard"]
            .with_user(admin)
            .create({"name": "Test Standard - D1ECFB7 Onchange", "code": "/"})
        )
        audit = (
            env["general_audit"]
            .with_user(admin)
            .create(
                {
                    "title": "Test General Audit - D1ECFB7 Onchange",
        ...
        account_group = (
            ...
            .with_user(admin)
            .create({"name": "Test Account Group - D1ECFB7 Onchange", "code": "/"})
        )
        account_type = (
            ...
            .create(
                {
                    "name": "Test Account Type - D1ECFB7 Onchange",
                    "code": "/",
                    "group_id": account_group.id,
                    # general_audit.standard_detail has no "balance" field, so a
                    # safe no-op avoids an unrelated crash in
    ...
    def test_onchange_general_audit_id_clears_account_type(self):
        ...
        ``general_audit_ws_c6c86fd`` / ``general_audit_ws_d4d1ac0`` worksheets
        in ``ssi_general_audit_worksheet_audit_procedure_recompute`` /
        ``ssi_general_audit_worksheet_audit_procedure_observation`` -- it is
