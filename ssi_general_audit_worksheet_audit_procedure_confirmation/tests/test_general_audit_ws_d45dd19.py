...
class TestGeneralAuditWsD45dd19(YamlTransactionCase):
    def test_general_audit_ws_d45dd19(self):
        self.run_yaml_scenario("test_data_general_audit_ws_d45dd19.yaml")
    ...
    def _create_worksheet_fixture(self):
        ...
        client = (
            ...
            .with_user(admin)
            .create(
                {"name": "Test Audit Client - D45DD19 Onchange", "is_company": True}
            )
        )
        accountant = (
            env["res.partner"]
            .with_user(admin)
            .create({"name": "Test Audit Accountant - D45DD19 Onchange"})
        )
        ...
        env["res.partner.id_number"].with_user(admin).create(
            {
                ...
                "name": "CPA-D45DD19-ONCHANGE-0001",
            }
        )
        account_type_set = (
            env["client_account_type_set"]
            .with_user(admin)
            .create({"name": "Test Account Type Set - D45DD19 Onchange", "code": "/"})
        )
        standard = (
            env["accountant.financial_accounting_standard"]
            .with_user(admin)
            .create({"name": "Test Standard - D45DD19 Onchange", "code": "/"})
        )
        audit = (
            env["general_audit"]
            .with_user(admin)
            .create(
                {
                    "title": "Test General Audit - D45DD19 Onchange",
        ...
        account_group = (
            ...
            .with_user(admin)
            .create({"name": "Test Account Group - D45DD19 Onchange", "code": "/"})
        )
        account_type = (
            ...
            .create(
                {
                    "name": "Test Account Type - D45DD19 Onchange",
                    "code": "/",
        ...
        ws_type = env.ref(
            "ssi_general_audit_worksheet_audit_procedure_confirmation.worksheet_type_d45dd19"
        )
    ...
    def test_onchange_general_audit_id_clears_account_type(self):
        ...
        ``general_audit_ws_aa899baf`` worksheet in
        ``ssi_general_audit_worksheet_audit_procedure_analytic`` -- it is a
