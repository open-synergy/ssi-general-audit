...
class TestGeneralAuditWsD4d1ac0(YamlTransactionCase):
    def test_general_audit_ws_d4d1ac0(self):
        self.run_yaml_scenario("test_data_general_audit_ws_d4d1ac0.yaml")
    ...
    def _create_worksheet_fixture(self):
        ...
        client = (
            ...
            .with_user(admin)
            .create(
                {"name": "Test Audit Client - D4D1AC0 Onchange", "is_company": True}
            )
        )
        accountant = (
            env["res.partner"]
            .with_user(admin)
            .create({"name": "Test Audit Accountant - D4D1AC0 Onchange"})
        )
        ...
        env["res.partner.id_number"].with_user(admin).create(
            {
                ...
                "name": "CPA-D4D1AC0-ONCHANGE-0001",
            }
        )
        account_type_set = (
            env["client_account_type_set"]
            .with_user(admin)
            .create({"name": "Test Account Type Set - D4D1AC0 Onchange", "code": "/"})
        )
        standard = (
            env["accountant.financial_accounting_standard"]
            .with_user(admin)
            .create({"name": "Test Standard - D4D1AC0 Onchange", "code": "/"})
        )
        audit = (
            env["general_audit"]
            .with_user(admin)
            .create(
                {
                    "title": "Test General Audit - D4D1AC0 Onchange",
        ...
        account_group = (
            ...
            .with_user(admin)
            .create({"name": "Test Account Group - D4D1AC0 Onchange", "code": "/"})
        )
        account_type = (
            ...
            .create(
                {
                    "name": "Test Account Type - D4D1AC0 Onchange",
                    "code": "/",
    ...
    def test_onchange_general_audit_id_clears_account_type(self):
        ...
        ``general_audit_ws_a145276`` worksheet in
        ``ssi_general_audit_worksheet_audit_procedure_inquiry`` -- it is a
        pre-existing pattern of the shared worksheet mixin's ``states``
