...
class TestGeneralAuditWsC6c86fd(YamlTransactionCase):
    def test_general_audit_ws_c6c86fd(self):
        self.run_yaml_scenario("test_data_general_audit_ws_c6c86fd.yaml")
    ...
    def _create_worksheet_fixture(self):
        ...
        client = (
            ...
            .with_user(admin)
            .create(
                {"name": "Test Audit Client - C6C86FD Onchange", "is_company": True}
            )
        )
        accountant = (
            env["res.partner"]
            .with_user(admin)
            .create({"name": "Test Audit Accountant - C6C86FD Onchange"})
        )
        ...
        env["res.partner.id_number"].with_user(admin).create(
            {
                ...
                "name": "CPA-C6C86FD-ONCHANGE-0001",
            }
        )
        account_type_set = (
            env["client_account_type_set"]
            .with_user(admin)
            .create({"name": "Test Account Type Set - C6C86FD Onchange", "code": "/"})
        )
        standard = (
            env["accountant.financial_accounting_standard"]
            .with_user(admin)
            .create({"name": "Test Standard - C6C86FD Onchange", "code": "/"})
        )
        audit = (
            env["general_audit"]
            .with_user(admin)
            .create(
                {
                    "title": "Test General Audit - C6C86FD Onchange",
        ...
        account_group = (
            ...
            .with_user(admin)
            .create({"name": "Test Account Group - C6C86FD Onchange", "code": "/"})
        )
        account_type = (
            ...
            .create(
                {
                    "name": "Test Account Type - C6C86FD Onchange",
                    "code": "/",
    ...
    def test_onchange_general_audit_id_clears_account_type(self):
        ...
        ``general_audit_ws_d4d1ac0`` / ``general_audit_ws_d45dd19`` worksheets
        in ``ssi_general_audit_worksheet_audit_procedure_observation`` /
